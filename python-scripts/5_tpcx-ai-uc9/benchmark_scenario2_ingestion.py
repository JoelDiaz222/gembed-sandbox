#!/usr/bin/env python3
"""
Benchmark 5 - TPCx-AI UC9, Scenario 2: Ingestion (image blobs stored in PG)
Embeds images and stores (blob + embedding) in PG; Qdrant/Chroma store embeddings only.
"""
import argparse
import os
import shutil
...
def create_chroma_client(base_path: str = "./chroma_bench_uc9_s2"):
...
def main():
    parser = argparse.ArgumentParser(description='Benchmark 5 UC9 - Scenario 2 Ingestion')
    ...
    if warmup_paths:
        ...
        warmup_pg_connection(conn_w2)
        setup_pg_schema(conn_w2)
        qd_w = create_qdrant_client()
        s2_ingest_qdrant(conn_w2, qd_w, warmup_paths, embed_client)
        if qd_w.collection_exists("faces"):
            qd_w.delete_collection("faces")
        qd_w.close()
        conn_w2.close()

        conn_w3, _ = connect_and_get_pid()
        warmup_pg_connection(conn_w3)
        setup_pg_schema(conn_w3)
    ...
    for size in test_sizes:
        ...
        # Qdrant (poly-store: blobs in PG, embeddings in Qdrant)
        conn2, pg_pid2 = connect_and_get_pid()
        ...
        # Chroma (poly-store: blobs in PG, embeddings in Chroma)
        conn3, pg_pid3 = connect_and_get_pid()
        warmup_pg_connection(conn3)
    ...
    output_dir = OUTPUT_DIR / "scenario2_ingestion"
