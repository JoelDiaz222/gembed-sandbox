...
COLOR_B6_ORT = '#ffa600'  # Amber – ONNX Runtime (ORT)
...
def generate_plots_b6(
    ...
    backend_label = {
        ...
        'ort': LABEL_MAP.get('ort', 'ONNX Runtime (ORT)'),
        'candle': 'Candle (CPU)',
    }
    backend_hatch = {
        'embed_anything': '',
    ...
    plt.tight_layout()
    plt.savefig(output_dir / f"throughput_b6_{timestamp}.pdf", format='pdf', bbox_inches='tight')
    plt.savefig(output_dir / f"throughput_b6_{timestamp}.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if 'embed_anything' in seen_backends and 'ort' in seen_backends:
        ...
        ax.set_xticklabels(seen_models, rotation=15, ha='right')
        ax.set_xlabel('Model')
        ax.set_ylabel(r'Speedup vs.\ ORT')
        ax.set_title(r'EmbedAnything (Candle, CUDA) Speedup over ORT')
        ax.grid(True, axis='y', linestyle='--', alpha=0.3, zorder=0)
        ax.set_axisbelow(True)
        plt.tight_layout()
        plt.savefig(output_dir / f"speedup_b6_{timestamp}.pdf", format='pdf', bbox_inches='tight')
        plt.savefig(output_dir / f"speedup_b6_{timestamp}.png", dpi=300, bbox_inches='tight')
        plt.close()
