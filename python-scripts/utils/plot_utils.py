...
def generate_plots_b6(
    ...
    def plot_resource_dual(cpu_key, mem_key, component_name, filename_suffix):
        ...
        for bi, backend in enumerate(seen_backends):
            offsets = [xi + (bi - (n_backends - 1) / 2) * bar_width for xi in x]
            cpu_heights, cpu_errs = [], []
            mem_heights, mem_errs = [], []
            for model_name in seen_models:
                method_name = f"{backend}_{model_name}"
                c_mean, c_std = _get(method_name, cpu_key)
                m_mean, m_std = _get(method_name, mem_key)
                cpu_heights.append(c_mean)
                cpu_errs.append(c_std)
                mem_heights.append(m_mean)
                mem_errs.append(m_std)
            ...
            ax_cpu.bar(offsets, cpu_heights, yerr=cpu_errs, **bar_kwargs)
            ax_mem.bar(offsets, mem_heights, yerr=mem_errs, **bar_kwargs)
...
def generate_plots_b8(df, output_dir, timestamp: str):
    ...
    One group per input size; bars represent mean values of the overhead
    components with ±1 σ error bars on the total wall-clock height.
    Also saves a LaTeX table of the results.
    """
    import numpy as np
    ...
    # Overhead components to stack (µs, warm-run model-cached path)
    # They must sum to approximately total_c_ext / ffi_roundtrip.
    components = [
        ...
        ('pure_embedding_us', r'EmbedAnything inference', '#ff6e54'),
    ...
    fig, ax = plt.subplots(figsize=(max(6, 1.8 * len(sizes)), 5))
    ...
    ax.set_title(r'Gembed Stack Overhead Breakdown per Batch Size')
