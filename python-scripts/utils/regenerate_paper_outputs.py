...
from utils.plot_paper_utils import generate_plots_from_csv
...
def find_benchmark_csvs():
    ...
    for root_dir, dirs, files in os.walk(project_root):
        ...
        if "output" in root_dir.split(os.sep) and "paper_output" not in root_dir.split(os.sep):
            for file in files:
...
def parse_csv_and_regenerate(file_path_str: str):
    ...
    # Copy the source CSV to the 'paper_output' directory for reference
