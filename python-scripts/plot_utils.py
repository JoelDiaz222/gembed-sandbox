...
def generate_plots(all_results: List[dict], output_dir: Path, timestamp: str, methods: List[str]):
    ...
    def plot_dual(cpu_key, mem_key, component_name, filename_suffix):
        ...
        for method in methods:
            ...
            y_errs = [r[method].get(f'{cpu_key}_iqr', r[method].get(f'{cpu_key}_std', 0)) for r in all_results if method in r]
        ...
        for method in methods:
            ...
            y_errs = [r[method].get(f'{mem_key}_iqr', r[method].get(f'{mem_key}_std', 0)) for r in all_results if method in r]
    ...
    def plot_throughput():
        ...
        for method in methods:
            ...
            y_errs = [r[method].get('throughput_iqr', r[method].get('throughput_std', 0)) for r in all_results if method in r]
